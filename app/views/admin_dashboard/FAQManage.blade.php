			...
			<a href = "admin-plus-faq/0" ><button class = "btn btn-success">Plus FAQ</button></a>
			  @if($FAQsUser != null)
				 @foreach($FAQsUser as $FAQUser)
				 		<tr>
				      		<td>{{$FAQUser->question}}</td>
				      		<td>{{$FAQUser->answer}}</td>
				      		<td>
				      		<p>Question</p>
				      		<form method = "post" action = "admin-change-content-faq">
			      				<input name = "type-FAQ" value = "question" hidden>
								<input name = "id" value = "{{$FAQUser->id}}" hidden>			      				
						 		<input name = "content-FAQ">
						 		
						 		<button type = "submit" class="btn btn-success">Change</button>
					 		</form>
					 		<p>Answer</p>
				      		<form method = "post" action = "admin-change-content-faq">
			      				<input name = "type-FAQ" value = "answer" hidden>
			      				<input name = "id" value = "{{$FAQUser->id}}" hidden>
						 		<input name = "content-FAQ">
					 		...
					 		<td>
					 			<form method = "post" action = "admin-delete-faq">
			 						<input name = "id" value = "{{$FAQUser->id}}" hidden>
			...
			<a href = "admin-plus-faq/1" ><button class = "btn btn-success">Plus FAQ</button></a>
			  @if($FAQsBuilder != null)
				 @foreach($FAQsBuilder as $FAQBuilder)
				 		<tr>
				      		<td>{{$FAQBuilder->question}}</td>
				      		<td>{{$FAQBuilder->answer}}</td>
				      		<td>
				      		<p>Question</p>
				      		<form method = "post" action = "admin-change-content-faq">
			      				<input name = "type-FAQ" value = "question" hidden>
